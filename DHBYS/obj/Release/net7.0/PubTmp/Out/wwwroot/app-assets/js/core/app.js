...
window.buttons = {
    icons: {
        ...
        detail: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>',
    ...
    edit: (url) => {
        return `<a href="${url}" class="btn btn-primary btn-icon btn-sm ms-auto edit">${buttons.icons.edit}</a>`;
    },
    delete: (url) => {
        return `<a href="${url}" class="btn btn-danger btn-icon btn-sm ms-auto delete">${buttons.icons.delete}</a>`;
    },
    deleteCurrentAccount: (id,code) => {
        return `<a class="btn btn-danger btn-icon btn-sm ms-auto delete-current-account" data-id="${id}" data-code="${code}">${buttons.icons.delete}</a>`;
    },
    detail: (url) => {
        return `<a href="${url}" class="btn btn-warning btn-icon btn-sm ms-auto detail">${buttons.icons.detail}</a>`;
    },
    list: (url) => {
        return `<a href="${url}" class="btn btn-info btn-icon btn-sm ms-auto detail">${buttons.icons.list}</a>`;
    },
    search: (url) => {
        return `<a href="${url}" class="btn btn-warning btn-icon btn-sm ms-auto detail">${feather.icons.search.toSvg()}</a>`;
    },
    file_text: (url) => {
        return `<a href="${url}" class="btn btn-secondary btn-icon btn-sm ms-auto detail">${feather.icons['file-text'].toSvg() }</a>`;
    }
};
...
(function (window, document, $) {
      ...
      if (value != '') {
        ...
        var $startList = '',
          ...
          $pageList =
            ...
            '<a href="#">' +
            '<h6 class="section-label mt-75 mb-0">Modüller</h6>' +
            '</a>' +
            '</li>',
          $activeItemClass = '',
          $bookmarkIcon = '',
  ...
  $(window).on('scroll', function () {
    if ($(this).scrollTop() > 400) {
      ...
      if ($(this).scrollTop() > 400) {
          $('.save-scroll-top').fadeIn();
      } else {
          $('.save-scroll-top').fadeOut();
      }
